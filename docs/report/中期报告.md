...
3. 点击"编辑"按钮修改图书信息
4. 点击"删除"按钮移除单本图书
5. 勾选多本图书后点击"批量删除"一键移除
6. 在搜索框输入关键词筛选图书
7. 通过状态下拉框按阅读状态筛选图书
8. 搜索与状态筛选可组合使用
9. 点击"导出 CSV"下载图书数据，导出内容与当前筛选结果一致
...
| 模块 | 文件 | 职责 |
|------|------|------|
| 图书列表模块 | `main.py - index()` | 展示所有图书，支持搜索和状态筛选 |
...
| 前端交互 | `index.html` | 页面展示、表单提交、AJAX 请求、批量操作、状态筛选 |
...
| 测试类型 | 测试范围 | 测试方法 |
|----------|----------|----------|
| 功能测试 | 增删改查、批量删除、搜索、状态筛选、统计、导出 | 手动执行测试用例 |
...
- ✅ 完成前端页面开发（列表展示、弹窗表单、统计看板、批量操作、状态筛选）
- ✅ 实现首次运行自动建表，简化部署流程
- ✅ 实现多条件组合筛选（搜索 + 状态）
- ✅ 系统可正常运行，核心功能完整
