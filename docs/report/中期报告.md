...
11. 图书较多时通过底部分页栏翻页浏览
12. 点击"导出 CSV"下载图书数据，导出全部数据不受分页影响
...
| 模块 | 文件 | 职责 |
|------|------|------|
| 图书列表模块 | `main.py - index()` | 展示所有图书，支持搜索、状态筛选、分页，计算阅读进度 |
...
| 前端交互 | `index.html` | 页面展示、表单提交、AJAX 请求、批量操作、状态筛选、进度条渲染、分页导航 |
...
books = conn.execute(
    f"SELECT ... ORDER BY book.id DESC LIMIT ? OFFSET ?",
...
def export_csv():
    # 支持 search 和 status 参数，导出全部匹配数据（不分页）
    output = io.StringIO()
...
| 测试类型 | 测试范围 | 测试方法 |
|----------|----------|----------|
| 功能测试 | 增删改查、批量删除、搜索、状态筛选、统计、导出、进度条、分页 | 手动执行测试用例 |
| 兼容性测试 | Chrome/Edge 浏览器 | 在不同浏览器中验证 |
| 边界测试 | 空数据、特殊字符、超长文本、分页边界 | 输入异常数据验证 |
...
- ✅ 完成前端页面开发（列表展示、弹窗表单、统计看板、批量操作、状态筛选、进度条、分页）
- ✅ 实现首次运行自动建表，简化部署流程
- ✅ 实现多条件组合筛选（搜索 + 状态）
